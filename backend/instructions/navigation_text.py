...
HYBRID ARCHITECTURE:
- For movement commands, call execute_navigation_sequence_tool() — this handles all retry logic,
...
MOVE PROCEDURE
1. Call execute_navigation_sequence_tool(asset_id, target_x, target_z, target_y).
...
2. For each waypoint, call execute_navigation_sequence_tool.

EXAMPLES:
- "Navigate BEACON-01 to building at (-15, -20)"
  -> execute_navigation_sequence_tool("BEACON-01", -15.0, -20.0)
  -> Report: "BEACON-01 arrived at (-15.0, 17.0, -20.0). Route: 3 waypoints, over strategy."

- "Move BEACON-01 to (10, 20, -5)"
  -> execute_navigation_sequence_tool("BEACON-01", 10.0, -5.0, 20.0)
