    ...
    <div className="flex flex-col gap-1.5">
      {entries.map(d => {
        ...
        return (
          <div
            key={d.asset_id}
            className={`rounded-md border border-l-2 bg-[linear-gradient(135deg,rgba(6,8,16,0.88),rgba(4,6,14,0.82))] p-[8px_12px] font-mono text-xs leading-[1.7] backdrop-blur-[12px] ${cardClass}`}
          >
            <div className="mb-1 flex items-center justify-between">
              <span className="font-bold tracking-[0.5px] text-[#ccdde8]">{d.asset_id}</span>
              <span className={`flex items-center gap-[5px] text-[11px] ${statusClass}`}>
                <span className={`h-1.5 w-1.5 rounded-full ${statusClass.replace('text-', 'bg-')} ${statusGlow}`} />
                {d.status}
              </span>
            </div>
            <div className="mb-1 flex items-center gap-1.5">
              <span className="text-[11px] text-[#7a8a9a]">BAT</span>
              ...
              <span className={`w-[34px] text-right text-[11px] ${batteryTextClassName}`}>{d.battery}%</span>
            </div>
            <div className="flex items-center justify-between text-[11px]">
              <span className="tracking-[0.3px] text-[#6a8aa0]">
